...
public class FingerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String intent_type = bundle.getString("type");
        String btn_action = bundle.getString("action");
        if (intent_type.equals("vehicle")) {
            vehicle = (Vehicle) bundle.getSerializable("vehicle");
        } else if (intent_type.equals("detainee")) {
            detainee = (Detainee) bundle.getSerializable("detainee");
        }
        ...
        if (!fingerprintManager.isHardwareDetected())
            ...
            if (!fingerprintManager.hasEnrolledFingerprints())
                ...
                if (cipherInit()) {
                    ...
                    if (intent_type.equals("vehicle")) {
                        helper.startAuthenticationVehicle(fingerprintManager, cryptoObject, vehicle, btn_action);
                    } else if (intent_type.equals("detainee")) {
                        helper.startAuthenticationDetainee(fingerprintManager, cryptoObject, detainee, btn_action);
                    }
                }
            }
        }
    ...
    private void genKey() {
        ...
        try {
            ...
            keyStore.load(null);
            keyGenerator.init(new KeyGenParameterSpec.Builder(KEY_NAME, KeyProperties.PURPOSE_ENCRYPT | KeyProperties.PURPOSE_ENCRYPT).setBlockModes(KeyProperties.BLOCK_MODE_CBC)
